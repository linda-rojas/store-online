...
export function Header() {
  return (
    <header className="sticky top-0 z-50">
      <nav
        className={`${styles.color_bg_red} flex items-center justify-between px-6 p-2 md:px-6 lg:px-12`}
      >
        <IoMenu className="h-9 w-9 text-white cursor-pointer md:hidden lg:hidden" />
        <IconAutocaucho />
        <div
          className={`${styles.gray_bg_input} hidden rounded-[10px] md:relative md:block md:top-0 lg:block`}
        >
          <input
            type="text"
      ...
      <section
        className={`${styles.blue_bg} h-[50px] w-full flex items-center justify-center `}
      >
        <div
          className={`${styles.gray_bg_input} relative rounded-[10px] w-[60%] md:hidden lg:hidden`}
        >
          <input
            type="text"
            placeholder="Buscar producto"
